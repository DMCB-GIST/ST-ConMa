...
from utils.pt_load_inference import GeneInference
...
# Output
OUTPUT_DIR = "./results/st_linear_probing/st_conma_ge_dlpfc"
...
def extract_embeddings_for_sample(
    ...
    """Extract gene embeddings for a single DLPFC sample"""
    ...
    # Filter spots that have annotations
    valid_sentences = []
    valid_labels = []
    valid_spot_ids = []
    valid_gene_ids = []
    ...
    # Extract embeddings
    embeddings = model.encode_gene_sentences_batch(
...
def load_all_data(model: GeneInference, df_gene: pd.DataFrame) -> Dict[int, Dict]:
    """Load all DLPFC data with gene embeddings and labels"""
...
def main():
    print("="*60)
    print("DLPFC Linear Probing with ST-ConMa Gene Encoder")
    print("="*60)
    print(f"\nConfiguration:")
    print(f"  Gene model: {GENE_MODEL_NAME}")
