...
from utils.pt_load_inference import VisionInference
...
# Output
OUTPUT_DIR = "./results/st_linear_probing/st_conma_ie_dlpfc"
...
def extract_embeddings_for_sample(
    ...
    """Extract embeddings for a single DLPFC sample"""

    misc_id = SAMPLE_ID_CONVERT[sample_id]
    image_pattern = os.path.join(IMAGE_DIR, f"{misc_id}_*.png")
    image_paths = sorted(glob.glob(image_pattern))

    if len(image_paths) == 0:
        print(f"Warning: No images found for {sample_id} ({misc_id})")
    ...
    for img_path in image_paths:
        ...
        # filename format: MISC1_AAACAAGTATCTCCCA-1.png
        spot_id = filename.replace(f"{misc_id}_", "").replace(".png", "")

        if spot_id in annotations:
            label_str = annotations[spot_id]
            if label_str in LABEL_MAP:  # Skip if label not in our mapping
                valid_paths.append(img_path)
                valid_labels.append(LABEL_MAP[label_str])
                valid_spot_ids.append(spot_id)

    if len(valid_paths) == 0:
        print(f"Warning: No valid annotated images for {sample_id}")
        return np.array([]), np.array([]), []
...
def load_all_data(model: VisionInference) -> Dict[int, Dict]:
    """Load all DLPFC data with embeddings and labels"""
...
def main():
    print("="*60)
    print("DLPFC Linear Probing with ST-ConMa Image Encoder")
    print("="*60)
    print(f"\nConfiguration:")
