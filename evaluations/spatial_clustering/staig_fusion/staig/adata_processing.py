...
class LoadSingle10xAdata:
    def __init__(self, path: str, n_top_genes: int = 3000, n_neighbors: int = 3, image_emb: bool = False, label: bool = True, filter_na: bool = True, npy_path: str = None, sample_name: str = None):
        self.path = path
    ...
    def preprocess(self):
        ...
        #         self.adata.var_names.isin(top_variable_genes)
        #     )
        #     sc.pp.normalize_total(self.adata, target_sum=1e4)
        #     sc.pp.log1p(self.adata)
        #     sc.pp.scale(self.adata, zero_center=False, max_value=10)
        ...
        #
        #     module_colors = np.unique(
        #         pyWGCNA_data.datExpr.var['moduleColors']
        #     ).tolist()
        #
        #     all_hub_genes = []
        #     for module_color in module_colors:
        #         df_hub_genes = pyWGCNA_data.top_n_hub_genes(
        #             moduleName=module_color,
        #             n=100
        #         )
        ...
        #
        #     self.adata.var['highly_variable'] = (
        #         self.adata.var_names.isin(all_hub_genes)
        #     )
        #     sc.pp.normalize_total(self.adata, target_sum=1e4)
        #     sc.pp.log1p(self.adata)
        #     sc.pp.scale(self.adata, zero_center=False, max_value=10)
        ...
        #
        #     self.adata.obs['total_counts'] = np.ravel(
        ...
        #
        #     self.adata.var['highly_variable'] = (
        #         self.adata.var_names.isin(top_genes_list)
        #     )
        #     sc.pp.normalize_total(self.adata, target_sum=1e4)
        #     sc.pp.log1p(self.adata)
        #     sc.pp.scale(self.adata, zero_center=False, max_value=10)
    ...
    def load_image_emb(self):
        # Load embeddings from npy_path if provided, otherwise from self.path
        if self.npy_path is not None and self.sample_name is not None:
            emb_path = os.path.join(self.npy_path, f'{self.sample_name}_embeddings.npy')
        else:
