...
def generate_pseudo_labels(img_emb, n_clusters=300):
    kmeans = KMeans(n_clusters=n_clusters, random_state=42).fit(img_emb) 
    pseudo_labels = kmeans.labels_ # 각 샘플이 어느 클러스터에 속하는지 라벨 추출
    return torch.tensor(pseudo_labels)


def adj_to_edge_index(adj): 
    row, col = torch.where(adj != 0)  # row: 출발 노드 인덱스, col: 도착 노드 인덱스
...
class STAIG:
    ...
    def draw_spatial(self, output_dir, p=''):
        import matplotlib.pyplot as plt
        import os
        os.makedirs(output_dir, exist_ok=True)
        if self.args.label == True and 'ground_truth' in self.adata.obs:
            fig, axes = plt.subplots(1, 2, figsize=(12, 5))
            sc.pl.spatial(self.adata, img_key='hires', size=1.6, color='ground_truth',
                          ax=axes[0], show=False, title='ground_truth')
            sc.pl.spatial(self.adata, img_key='hires', size=1.6, color='domain',
                          ax=axes[1], show=False, title='domain')
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, 'spatial_' + p + str(self.args.slide) + '.png'), dpi=300, bbox_inches='tight')
            plt.close()
        else:
            fig, ax = plt.subplots(1, 1, figsize=(6, 5))
            sc.pl.spatial(self.adata, img_key='hires', size=1.6, color='domain',
                          ax=ax, show=False, title='domain')
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, 'spatial_' + p + str(self.args.slide) + '.png'), dpi=300, bbox_inches='tight')
            plt.close()
    ...
    def draw_umap(self, output_dir):
        ...
        if self.args.label == True and 'ground_truth' in self.adata.obs:
            fig, axes = plt.subplots(1, 2, figsize=(12, 5))
            sc.pl.umap(self.adata, color='ground_truth', ax=axes[0], show=False, title='ground_truth')
            sc.pl.umap(self.adata, color='domain', ax=axes[1], show=False, title='domain')
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, 'umap_' + str(self.args.slide) + '.png'), dpi=300, bbox_inches='tight')
            plt.close()
        else:
            fig, ax = plt.subplots(1, 1, figsize=(6, 5))
            sc.pl.umap(self.adata, color='domain', ax=ax, show=False, title='domain')
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, 'umap_' + str(self.args.slide) + '.png'), dpi=300, bbox_inches='tight')
            plt.close()
