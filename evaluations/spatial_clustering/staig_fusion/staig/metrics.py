...
ro.r.source(os.path.join(_CURRENT_DIR, 'batchKL.R'))
ro.r.source(os.path.join(_CURRENT_DIR, 'calLISI.R'))
...
def find_resolution(adata_, n_clusters, random=42):
    adata = adata_.copy()
    obtained_clusters = -1
    iteration = 0
    resolutions = [0., 1000.]
...
def calulate_ari_nmi(adata_integrated,n_cluster=4):
    sc.pp.neighbors(adata_integrated,random_state=42)
    reso=find_resolution(adata_integrated,n_cluster,42)
    sc.tl.louvain(adata_integrated,reso,random_state=42)
